...
class AppotaView extends Component {
    ...
    componentDidMount() {
        ...
        GoogleAnalytics.trackScreenView('ga_cashout_otp');
    }
    ...
    ConfirmOTP(otp) {
        ...
        RequestHelper.ConfirmOtp(access_token, this.props.transaction_id, otp, 'cashout')
            .then((data) => {
                this.setState({ isLoading: false });
                GoogleAnalytics.trackEvent('ga_cashout', 'Confirm OTP');
                appsFlyer.trackEvent('af_cashout_otp', {}, () => { }, () => { });
                AppEventsLogger.logEvent('fb_cashout_otp', 1);
                if (data.status == 200) {
                    Actions.popup({ title: Lang.thong_bao(), content: Lang.rut_tien_thanh_cong(), flag: 'success', onPress_Ok: () => popToHome() });
                }
                else {
