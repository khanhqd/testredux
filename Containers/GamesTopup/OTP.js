...
class AppotaView extends Component {
    ...
    componentDidMount() {
        ...
        GoogleAnalytics.trackScreenView('ga_games_topup_otp');
    }
    ...
    ConfirmOTP(otp) {
        ...
        RequestHelper.ConfirmOtp(access_token, this.props.transaction_id, otp, 'games')
            .then((data) => {
                Keyboard.dismiss()
                this.setState({ isLoading: false })
                if (data.status == 200) {
                    GoogleAnalytics.trackEvent('ga_games_topup', 'Confirm OTP');
                    appsFlyer.trackEvent('af_games_topup_otp', {}, () => { }, () => { });
                    AppEventsLogger.logEvent('fb_games_topup_otp', 1);

                    Actions.popup({ title: Lang.thong_bao(), content: Lang.nap_game2() + ' ' + Lang.thanh_cong().toLowerCase(), flag: 'success', onPress_Ok: () => popToHome() })
                }
                else {
