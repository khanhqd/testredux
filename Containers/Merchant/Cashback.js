...
class Cashback extends Component {
    ...
    Cashback() {
        ...
        setTimeout(() => {
            RequestHelper.Cashback(access_token, this.state.phone, 'money', this.state.amount, this.props.cashback_percent, this.state.message)
                .then((data) => {
                    if (data.status == 200) {
                        const eventName = "af_merchant_cashback";
                        appsFlyer.trackEvent(eventName, {}, () => { }, () => { });
                        var trans = JSON.parse(data._bodyText)
                        if (user_infor.pay_verify_method == 'NONE')
                            Actions.protected_mc({ transaction_id: trans.transaction_id, transaction_type: 'cashback' })
                        else
                            Actions.merchant_otp({ transaction_id: trans.transaction_id })
                ...
                <ScrollView style={styles.FormControl_Scroll} keyboardShouldPersistTaps='always' showsVerticalScrollIndicator={false}>
                    <View style={styles.FormControl_Title}>
                        <Text style={styles.FormControl_TitleText}>
                            {Lang.hoan_tien().toUpperCase()} {Lang.thong_tin().toUpperCase()}
                        </Text>
                    </View>
                        ...
                        <View style={styles.FormControl_Profile}>
                            ...
                            <View style={styles.FormControl_Input_Form}>
                                ...
                                <TextInput
                                    ...
                                    onBlur={() => this._onBlur()}
                                    onFocus={() => this._onFocus()}
                                    onSubmitEditing={() => this.confirmCashback()}
                            ...
                            <View style={styles.FormControl_Line}></View>
                            <View style={styles.FormControl_Input_Form}>
                                <Text style={styles.FormControl_Active}>{Lang.so_tien_hoan_lai()} ({Utils.toFixed(this.state.cashback_percent, 2)}%)</Text>
                    ...
                    <View style={styles.FormControl_Note}>
                        <Text style={styles.FormControl_NoteText}>
                            {Lang.note_cashback_2()}
                        </Text>
                    </View>
                </ScrollView>
                <View style={styles.FormControl_Button}>
                    {renderIf(this.state.phone != '' && this.state.amount != '')(
                        <TouchableOpacity style={styles.FormControl_ButtonTouch} onPress={() => this.confirmCashback()}>
                            <Text style={styles.FormControl_ButtonText}>
                                {Lang.hoan_tien().toUpperCase()}
                            </Text>
                        </TouchableOpacity>
                    )}
                    {renderIf(this.state.phone == '' || this.state.amount == '')(
