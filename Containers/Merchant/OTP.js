...
var Home_OTP = React.createClass({
    ...
    ConfirmOTP(otp) {
        GoogleAnalytics.trackEvent('OTP', 'Confirm OTP cashback');
        this.setState({ isLoading: true });
        RequestHelper.ConfirmOtp(access_token, this.props.transaction_id, otp, 'cashback')
            .then((data) => {
                this.setState({ isLoading: false });
                const eventName = "af_cashback_otp";
                appsFlyer.trackEvent(eventName, {}, () => { }, () => { });
                if (data.status == 200) {
                    Actions.popup({ title: Lang.thong_bao(), content: Lang.hoan_tien() + ' ' + Lang.thanh_cong().toLowerCase(), flag: 'success', onPress_Ok: () => popToHome() });
                }
                else {
