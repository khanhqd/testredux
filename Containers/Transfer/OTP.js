...
class AppotaView extends Component {
    ...
    componentDidMount() {
        ...
        GoogleAnalytics.trackScreenView('ga_transfer_otp');
    }
    ...
    ConfirmOTP(otp) {
        ...
        RequestHelper.ConfirmOtp(access_token, this.props.transaction_id, otp, 'transfer')
            .then((data) => {
                Keyboard.dismiss()
                this.setState({ isLoading: false })
                if (data.status == 200) {
                    GoogleAnalytics.trackEvent('ga_transfer', 'Confirm OTP');
                    appsFlyer.trackEvent('af_transfer_otp', {}, () => { }, () => { });
                    AppEventsLogger.logEvent(`fb_transfer_otp`, 1);
                    Actions.popup({ title: Lang.thong_bao(), content: Lang.chuyen_tien() + ' ' + Lang.thanh_cong().toLowerCase(), flag: 'success', onPress_Ok: () => popToHome() });
                }
                else {
